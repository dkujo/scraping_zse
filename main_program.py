import sys
sys.path.append(r'C:\Users\Domagoj\scraping_zse')
...
driver_stock = webdriver.Chrome('C:/Users/Domagoj/scraping_zse/chromedriver.exe',options=option)

stock_tickers_and_ISIN = my_scraping.scrape_stock_symbols(driver_stock, url_stocks_symbols)
...
stock_history_data = my_scraping.scrape_history(driver_stock, url_stock_history)
...
url_stock_history2 = 'https://zse.hr/hr/papir/310?isin=' + stock_ISIN2 + \
    ...
    '&date_to=' + today_date + '&model=ALL'
# call scraping function 
stock_history_data2 = my_scraping.scrape_history(driver_stock, url_stock_history2)


# extract time and price from stock history data
stock_time, stock_price = my_scraping.extract_data_from_df(stock_history_data['Datum'],stock_history_data['Zadnja'])
stock_history_data['Datum'] = [x.strftime('%Y-%m-%d') for x in stock_time][::-1]

# extract time and price from stock2 history data
stock_time2, stock_price2 = my_scraping.extract_data_from_df(stock_history_data2['Datum'],stock_history_data2['Zadnja'])
...
# call scraping function
crobex_history_data = my_scraping.scrape_history(driver_stock, url_crobex_history)

# extract time and price from crobex history data
crobex_time, crobex_value = my_scraping.extract_data_from_df(crobex_history_data['Datum'],crobex_history_data['Zadnja'])

# plot crobex history data
my_scraping.my_plot(crobex_time, crobex_value, 'Time', 'Crobex value', 'Index Crobex', 'Index history')
